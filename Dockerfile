...
RUN apk add --no-cache --virtual .builder tar && \
    apk add --no-cache su-exec \
                       openjdk8-jre && \
    mkdir /usr/share/elasticsearch/ && curl --location https://download.elastic.co/elasticsearch/release/org/elasticsearch/distribution/tar/elasticsearch/2.3.5/elasticsearch-2.3.5.tar.gz \
